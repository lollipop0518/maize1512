...
1. 矩阵索引化 ：
   - Excel 读入 R 后默认是数据框。为了能通过 `dist_mat["SampleA", "SampleB"]` 这种方式快速查找到距离值，我将数据框的第一列转换成了**行名**。
2. **交错构建 (`output_list`)**：
   - 图片的要求是一行文本、一行数字。
   - 在 `for` 循环中，针对每一个样本，我构建了两个向量：
     - `row_names`: 包含样本ID和20个邻居ID。
     - `row_values`: 包含一个空白格（对应ID列）和20个查找到的距离值。
   - 然后将这两个向量依次加入列表。
3. **查找距离 (`dist**查找距离 (`dist_mat[row, col]`)**：
   - 代码核心在于 `dist_mat[focal`dist_mat[focal_id, nb_id]`。它利用行名和列名直接定位 Excel 表格中的单元格数据。
4. **格式对齐**：
   - 图片中的第一列是“序号”，代码最后通过 `cbind` 添加了这一列。
   - 图片中的列名是 `材料.1` 到 `材料.20`，代码通过 `paste0` 自动生成了这些列名。
...
> ⚠️ Script 2 requires the use of the genetic distance and the top 20 files generated by Script 1 as input.
